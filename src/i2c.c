...
void I2C_Init(void) {
    ...
    SIM->SCGC5 |= SIM_SCGC5_PORTC_MASK; /* clock for PORTB */
    
    PORTC->PCR[SCL] |= PORT_PCR_MUX(2); /* I2C0 SCL    (PTB3) */
    PORTC->PCR[SDA] |= PORT_PCR_MUX(2); /* I2C0 SDA    (PTB4) */
    
    I2C0->C1 &= ~(I2C_C1_IICEN_MASK);   /* disable module during modyfications*/
    
    /* Set baud rate = BusClock / (2^MULT * SCLdivider) */
    I2C0->F  |= I2C_F_MULT(0x1);        /* MULT = 0,1,2 */
    I2C0->F  |= I2C_F_ICR(0x1B);        /* SCLdivider */ 
...
void i2c_wait(void) {
    ...
    while( ((I2C0->S & I2C_S_IICIF_MASK)==0) && (timeout<10000) ) 
        timeout++;
    if( timeout >= 10000 )                           
        error |= I2C_ERR_TIMEOUT; 
    if( (I2C0->S & I2C_S_RXAK_MASK)==1 ) 
        error |= I2C_ERR_NOACK;
